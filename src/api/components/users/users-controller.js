...
async function getUsers(request, response, next) {
  try {
    ...
    if (search) {
      ...
      if (field === 'email') {
        users = users.filter((user) => user.email.includes(cariValue));
      } else if (field === 'name') {
        users = users.filter((user) => user.name.includes(cariValue));
      }
    }
    ...
    const count = users.length; //banyaknya users
    ...
    const pageUsers = users.slice(start_index, end_index);
